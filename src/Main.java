...
public class Main {
	...
	static {
		...
		/*
		 * commands.put("printAllPatients", (section, args) -> { for(Department d :
		 * hospital.getDepartmentsById().values()) {
		 * section.write(d.printAllPatients()); } });
		 * 
		 * commands.put("printAllDoctors", (section, args) -> { for(Department d :
		 * hospital.getDepartmentsById().values()) { section.write(d.printAllDoctors());
		 * } });
		 * 
		 * commands.put("printAllNurses", (section, args) -> { for(Department d :
		 * hospital.getDepartmentsById().values()) { section.write(d.printAllNurses());
		 * } });
		 */
	...
	public static void main(String[] args) throws IOException {
		...
		try{
			...
			AutoUtils.makeFile(OUTPUT_FILE,document.exportNormal());
			AutoUtils.makeFile(AP_FILE,document.exportSections());
